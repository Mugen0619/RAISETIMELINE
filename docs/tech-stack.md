...
| @axe-core/playwright | 4.13系 | Playwrightと組み合わせた主要画面のアクセシビリティ検査に使用(axe-core本体もdependenciesとして同時に導入される) |
| k6 | 2.2系 | 代表的なAPI(タイムライン取得・フォロー中タイムライン取得・ログイン)の負荷試験に使用。JS/TSでシナリオを書けPlaywrightと言語を揃えられる。個人開発規模を想定しローカル実行のみを対象とする(詳細は[k6/README.md](../k6/README.md)を参照) |
...
| 技術 | バージョン | 備考 |
|---|---|---|
| GitHub Actions | - | push・PR時にバックエンド/フロントエンドのテスト・Lint・E2Eテスト(ユーザージャーニー+アクセシビリティ検査)を自動実行する(`.github/workflows/ci.yml`)。詳細はリポジトリルートの[README.md](../README.md)の「CI」節を参照 |
