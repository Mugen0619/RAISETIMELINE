...
[GitHub Actions](.github/workflows/ci.yml)で、push・PR時に以下を自動実行する。

- バックエンドの単体・統合テスト(`./gradlew test`)、Lint(Checkstyle)
- フロントエンドの単体・コンポーネントテスト(`npm test`)、Lint
- PlaywrightのE2Eテスト(ユーザージャーニー+アクセシビリティ検査。CI上でPostgreSQL・バックエンド・フロントエンドを一時的に起動してから実行する)

負荷試験(k6)・ブラウザパフォーマンステストは対象外で、引き続きローカルでのみ実行する(理由はそれぞれ[k6/README.md](k6/README.md)・後述の「ブラウザパフォーマンステスト」節を参照)。本番環境へのデプロイの自動化(CD)は今回のスコープ外。

## E2Eテスト

代表的なユーザージャーニー(新規登録〜投稿、投稿詳細でのコメント・いいね・フォロー、ユーザー検索)、主要画面のアクセシビリティ検査、主要画面のブラウザパフォーマンス計測をPlaywrightで行う。
単体・結合・コンポーネントテストとは異なり、バックエンド(実DB込み)とフロントエンドを実際に起動した状態で実行する。
ユーザージャーニー・アクセシビリティ検査はGitHub Actions上でも自動実行するが(詳細は「CI」節を参照)、ブラウザパフォーマンス計測は共有ランナーの性能変動によりしきい値判定が信頼できないため、CI対象外としローカルでのみ実行する。
